...
import { decodePayload } from "./store/Auth";
...
function createPage(
  root: any,
  { style, link, script }: { style: string; link: string; script: string }
) {
  return `
    <!DOCTYPE HTML>
    <html>
...
async function serverRender(ctx: Koa.ParameterizedContext) {
    ...
    Auth: {
      ...
      data:
        (ctx.cookies.get("auth_token") &&
          decodePayload(ctx.cookies.get("auth_token") as string)) ||
        null
    }
  });
  ...
  await Promise.all(
    Object.entries(prefetchContext.promises).map(([key, value]) => {
      return (async () => {
  ...
  const preloadScript = `<script>window.__PRELOAD_SERVER__=${JSON.stringify(
    prefetchContext.preloads
  ).replace(/</g, "\\u003c")}</script>`;
  const preloadStateScript = `<script>window.__PRELOAD_STATE__=${JSON.stringify(
    store.getState()
  ).replace(/</g, "\\u003c")}</script>`;
